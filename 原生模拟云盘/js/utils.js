...
function mousewheel(element,upFn,downFn){
	//不等于元素
		...
		function fn(ev){
			//console.log("滚动了")
			var e = ev||event
			console.log(e.wheelDelta) //IE和谷歌支持 上滚正数 下滚负数
			console.log(e.detail) //火狐支持 上滚负数 下滚正数
			var direction = true //自定义属性方向默认为上
			//判断是什么浏览器使什么东西
			if(e.wheelDelta){
...
function collision(element1,element2){
	...
	if(element1Rect.right<element2Rect.left ||element1Rect.bottom<element2Rect.top||element1Rect.left>element2Rect.right||element1Rect.top>element2Rect.bottom){
		return false
	}else{
		return true
	}
